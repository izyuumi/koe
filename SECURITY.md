...
- **No direct network calls**: Koe itself makes no network calls. On-device recognition is fully offline. Cloud recognition goes through Apple's Speech API.
- **No telemetry**: No data is collected or transmitted.
- **Speech helper**: An isolated Swift subprocess. Communicates with the main app only via stdout (one-way text protocol).
